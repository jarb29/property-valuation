...
def setup_paths():
    ...
    sys.path.insert(0, str(streamlit_dir.parent / "src"))
    sys.path.insert(0, str(streamlit_dir.parent))
...
def prepare_best_model():
    """Main function to prepare the best model for Streamlit app"""
    # Setup
    streamlit_dir = setup_paths()
    from config import MODEL_METRIC
    
    # Paths
    pipeline_models_dir = streamlit_dir.parent / "outputs" / "pipeline" / "models"
    bestmodel_dir = streamlit_dir / "models" / "bestmodel"
